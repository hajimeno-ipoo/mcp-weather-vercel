...
##エージェントループのルール:
1. コードの追加・変更後は、必ず npm test を実行すること。
2. もしテストが失敗した場合は、エラーログを解析し、修正案を作成して再度テストを実行せよ。
3. npm run lint を実行し、警告がゼロになるまで自己修正を繰り返すこと。
4. すべての検証がパスするまで、タスクを完了と報告してはならない
