...
import { z } from "zod";
import type { GeoCandidate, OpenMeteoForecastResponse } from "./types";
import { APIError } from "./types";
import { forecastCache, generateForecastKey } from "./cache";
import { ICON_PNG_BASE64 } from "./iconData";
import { geocodeCandidates as geocodeCandidatesImpl } from "./geocodeCandidates";
...
// NOTE: 候補検索ロジックは `./geocodeCandidates` に集約（ローカルJP→Open‑Meteo ja→en）
const geocodeCandidates = (place: string, count: number) =>
  geocodeCandidatesImpl(place, count, CONFIG.GEOCODING_API_URL);
